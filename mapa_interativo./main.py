...
estados = ['AC', 'AL', 'AM', 'AP', 'BA', 'CE', 'DF', 'ES', 'GO', 'MA', 'MT',
 ...
 'SP', 'SE', 'TO']
capitais = ['Rio Branco', 'Maceió', 'Manaus', 'Macapá', 'Salvador', 
...
'Palmas']
latitudes = [-8.77, -9.62, -3.47, 0.038, -12.97, -3.71, -15.83, -20.32, -16.64,
...
for i in range(len(capitais)):
	fig.add_trace(
		...
		mode = 'markers',
		marker = dict(
			size = 8, color = 'yellow', line = dict(width = 1, color = 'black'))
		)
	)
...
fig.update_layout(
	...
	geo = dict(
		scope = 'south america',showland = True,
		...
		rotation = dict(lon = -60, lat = -10, roll = 0)),
